...
def _user_prompt(inp: ReviewInput) -> str:
    ...
    if kind and kind not in ("brand", "own", "company"):
        account = (f"\nACCOUNT TYPE: {inp.target_type} — this account is not the brand. It is a person "
                   ...
                   "\n\nWork these three in order before any claims assessment, and say in "
                   "violation_reason which of them you found:"
                   "\n  1. ENDORSEMENT — does the person lend professional standing to a cosmetic? "
                   "Their own title or credentials anywhere in the post, a clinic or hospital "
                   "setting, scrubs or a white coat, a stethoscope, 'Dr' in the handle or the "
                   "displayed name, or wording inviting the reader to trust the product because of "
                   "who is saying it. This is s.4.1 and it does not need a claim to be a breach."
                   "\n  2. PARTNERSHIP — is this a commercial arrangement? #ad, #sponsored, "
                   "'paid partnership', 'collab', an affiliate or discount code, a swipe-up or "
                   "order link, 'gifted', 'thank you to @brand', or the brand tagged as a partner. "
                   "Record it whether or not it is disclosed: an undisclosed one is the more "
                   "serious finding, and a disclosed one still carries s.4.1 where the person is a "
                   "health professional."
                   "\n  3. PRODUCT SHOWN — is an identifiable cosmetic product visible or named? "
                   "Packaging on camera, a product held or applied, a legible brand or product "
                   "name on the artwork, a product tag or shopping sticker. Name it in "
                   "product_name. A post by such a person that shows no identifiable product and "
                   "makes no claim is Acceptable — say so plainly rather than reaching for a "
                   "finding."
                   "\n\nAn endorsement or an undisclosed partnership around an identifiable "
                   "cosmetic is Unacceptable under s.4.1 even when every individual claim in the "
                   "caption would pass the Part 8 tables.")
    else:
        account = ""
